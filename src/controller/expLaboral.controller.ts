...
export const putExpLaboral = async (req: Request, res: Response): Promise<Response> => {
    ...
    let expLab: any = await helperExpLaboral.get(req.body.id);
    if(!expLab) return res.status(404).json({message: "No existe Experiencia Laboral"});
    Object.assign(expLab, req.body);
    if(validacion(expLab)) return res.status(400).json({message: "Valores incorrectos"});
...
export const  deleteExpLaboral = async (req: Request, res: Response): Promise<Response> => {
    if(!req.params.id) return res.status(400).json({message: "No se ingreso id"});
    let expLab: any = await helperExpLaboral.get(req.params.id);
    if(!expLab) return res.status(404).json({message: "No existe Experiencia Laboral"});

    let jwtauth = JSON.parse(req.params.jwtauth);
    if(expLab.postulante.id!=jwtauth.usuario) return res.status(403).json({message: "No tienes los permisos"});
